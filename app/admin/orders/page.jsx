...
export default function AdminOrdersPage() {
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    const itemMatch = order.items.some((item) =>
      [item.name, item.description].join(" ").toLowerCase().includes(searchTerm.toLowerCase())
    );
  ...
  return (
    <div className="orders-container">
      <h1 className="admin-title">Manage Orders ({filteredOrders.length || 0})</h1>
      <Input
        type="text"
        placeholder="Search..."
        value={searchTerm}
      ...
      />
      <select value={selectedRange} onChange={(e) => setSelectedRange(e.target.value)} className="date-filter">
            ...
            <tbody>
              {filteredOrders.map((order) => (
                <tr key={order.id}>
                  ...
                  <td>
                    {updatingOrderId === order.id ? <div className="spinner-small"></div> : (
                      <select value={order.order_status} onChange={(e) => handleOrderStatus(e, order.id)}>
