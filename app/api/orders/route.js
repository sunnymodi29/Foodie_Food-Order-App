...
export async function POST(request) {
  ...
  if (!orderData?.items?.length) {
    return NextResponse.json({ message: "Missing order items." }, { status: 400 });
  }

  const { name, email, street, city, postal_code } = orderData.customer;
  if (!email || !name || !street || !city || !postal_code) {
    return NextResponse.json({ message: "Missing customer info." }, { status: 400 });
  }
  ...
  try {
    await pool.query(
      ...
      [orderId, email, name, street, city, postal_code, JSON.stringify(orderData.items), orderData.user_id]
    );
